...
class _TaxSettingsScreenState extends State<TaxSettingsScreen> {
  ...
  Future<void> _loadSettings() async {
    ...
    try {
      ...
      final res = await FunctionsService.call('getTaxSettings', {'businessId': bizId});
              ...
              children: [
                const Row(
                  children: [
                    ...
                    SizedBox(width: 12),
                    Text('KRA eTIMS Integration', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 8),
                const Text('Configure electronic tax invoice management for your branch.', style: TextStyle(color: Colors.grey)),
                const Divider(height: 32),
                
                SwitchListTile(
                  title: const Text('Enable eTIMS Registration', style: TextStyle(fontWeight: FontWeight.bold)),
                  subtitle: const Text('Automatically send all completed sales to the revenue authority via the OSC API.'),
                  value: _eTimsEnabled,
