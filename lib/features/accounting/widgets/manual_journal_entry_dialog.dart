...
class ManualJournalEntryDialog extends StatefulWidget {
  ...
  @override
  State<ManualJournalEntryDialog> createState() => _ManualJournalEntryDialogState();
}
...
class _ManualJournalEntryDialogState extends State<ManualJournalEntryDialog> {
  ...
  void _submit() async {
    ...
    if (_descCtrl.text.isEmpty || _selectedDebitAccount == null || _selectedCreditAccount == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please fill all fields')));
      return;
    }

    if (debitAmt <= 0 || creditAmt <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Amounts must be greater than zero')));
      return;
    }

    if ((debitAmt - creditAmt).abs() > 0.01) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Debits must equal Credits')));
      return;
    }
    ...
    try {
      final lines = [
        {'accountId': _selectedDebitAccount!.id, 'debit': debitAmt, 'credit': 0},
        {'accountId': _selectedCreditAccount!.id, 'debit': 0, 'credit': creditAmt},
      ];
      await context.read<AccountingProvider>().postManualJournal(_descCtrl.text, lines);
      if (mounted) {
        Navigator.pop(context, true);
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Journal Entry posted successfully')));
      }
    } catch (e) {
      if (mounted) {
    ...
    return AlertDialog(
        ...
        child: Column(
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Description (e.g., Owner Investment)'),
            ),
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Debit Amount', prefixText: 'KES '),
            ...
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'Credit Amount', prefixText: 'KES '),
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
            ),
          ],
        ),
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          child: _isSubmitting ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Post Entry'),
        ),
      ],
    );
  }
