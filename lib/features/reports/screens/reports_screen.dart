            ...
            child: Column(
              ...
              children: [
                ...
                if (_stats != null) ...[
                  ...
                  if ((_stats!['expenseByCategory'] as Map?)?.isNotEmpty ==
                      true)
                    _ExpenseBreakdownCard(
                        stats: _stats!, fmt: _fmt, theme: theme),
                ],
              ],
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return _ReportCard(
      ...
      child: Column(children: [
        _StatRow('Revenue', fmt.format(revenue),
            color: AppColors.chartGreen, theme: theme),
        const SizedBox(height: 8),
        _StatRow('Cost of Goods', fmt.format(cogs),
            color: theme.colorScheme.onSurfaceVariant, theme: theme),
        const SizedBox(height: 8),
        _StatRow('Expenses', fmt.format(expenses),
            color: AppColors.chartRed, theme: theme),
...
class _SalesCard extends StatelessWidget {
  ...
  const _SalesCard(
      {required this.stats, required this.fmt, required this.theme});
  ...
  Widget build(BuildContext context) {
    ...
    return _ReportCard(
      ...
      child: Column(children: [
        ...
        _StatRow('Cash', fmt.format(cashTotal),
            color: AppColors.chartBlue, theme: theme),
        const SizedBox(height: 8),
        _StatRow('M-Pesa', fmt.format(mpesaTotal),
            color: AppColors.success, theme: theme),
        const SizedBox(height: 8),
        _StatRow('Credit', fmt.format(creditTotal),
...
class _ExpenseBreakdownCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final byCategory =
        (stats['expenseByCategory'] as Map?)?.cast<String, dynamic>() ?? {};
    final total =
        byCategory.values.fold<double>(0, (s, v) => s + (v as num).toDouble());
...
class _StatRow extends StatelessWidget {
  ...
  const _StatRow(this.label, this.value,
      {this.color, this.large = false, required this.theme});
