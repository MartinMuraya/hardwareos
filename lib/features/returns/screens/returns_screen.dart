              ...
              Expanded(
                child: _returns.isEmpty && !_loading
                    ? const EmptyState(
                        ...
                        subtitle:
                            'Process customer returns from completed sales.',
                      )
                    : RefreshIndicator(
                        onRefresh: _load,
...
class _ReturnCard extends StatelessWidget {
  ...
  const _ReturnCard(
      {required this.ret, required this.fmt, required this.theme});
...
class _ProcessReturnDialogState extends State<ProcessReturnDialog> {
  ...
  Future<void> _loadSales() async {
    try {
      final bizId = context.read<AuthProvider>().businessId!;
      final data = await FunctionsService.call(
          'getSales', {'businessId': bizId, 'limit': 100});
  ...
  Future<void> _submit() async {
    ...
    if (items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Select at least one item with quantity > 0')),
      );
      return;
    }
    ...
    return Dialog(
      ...
      child: ConstrainedBox(
        constraints: BoxConstraints(
            maxWidth: 550, maxHeight: MediaQuery.of(context).size.height * 0.9),
        child: Padding(
                            ...
                            child: ListView.separated(
                              ...
                              itemBuilder: (_, i) {
                                ...
                                return ListTile(
                                  dense: true,
                                  title: Text(
                                      '${s.items.length} items · ${DateFormat('MMM d, HH:mm').format(s.createdAt)}',
                                      style: const TextStyle(fontSize: 13)),
                                  subtitle: Text(
                                      'KES ${s.total.toStringAsFixed(0)} · ${s.paymentMethod}',
                                      style: const TextStyle(fontSize: 11)),
                                  trailing: Text('${s.items.length} items',
                                      style: const TextStyle(fontSize: 11)),
                                  onTap: () {
                                    setState(() {
                                      _selectedSale = s;
                        ...
                        Row(children: [
                          Expanded(
                              child: Column(
                                  ...
                                  children: [
                                Text(
                                    '${_selectedSale!.items.length} items · ${_selectedSale!.paymentMethod}',
                                    style: const TextStyle(
                                        fontWeight: FontWeight.w600)),
                                Text(
                                    'Total: KES ${_selectedSale!.total.toStringAsFixed(0)}',
                            ...
                            child: Row(children: [
                              Expanded(
                                  child: Column(
                                    ...
                                    Text(
                                        'Sold: ${item.quantity} · KES ${item.sellingPrice.toStringAsFixed(2)}',
                                        style: const TextStyle(fontSize: 11)),
                                  ])),
                              SizedBox(
                                width: 100,
