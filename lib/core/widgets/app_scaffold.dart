...
class _AppScaffoldState extends State<AppScaffold> {
    ...
    return [
      ...
      if (isManager)
        const _NavItem(
            ...
            route: '/inventory'),
      const _NavItem(
          icon: Icons.point_of_sale_rounded, label: 'Sales', route: '/sales'),
      const _NavItem(
          icon: Icons.sync_problem_rounded,
          label: 'Sync Conflicts',
          route: '/sales/conflicts'),
      if (isManager)
        const _NavItem(
            icon: Icons.shopping_basket_rounded,
            label: 'Online Orders',
            route: '/sales/online-orders'),
      if (isManager)
        const _NavItem(
            ...
            route: '/expenses'),
      const _NavItem(
          icon: Icons.people_rounded, label: 'Customers', route: '/customers'),
      const _NavItem(
      ...
      if (isManager)
        const _NavItem(
            icon: Icons.store_rounded, label: 'Suppliers', route: '/suppliers'),
      if (isManager)
        const _NavItem(
            icon: Icons.account_balance_wallet_rounded,
            label: 'Supplier Debt',
      ...
      if (isOwner)
        const _NavItem(
            icon: Icons.people_rounded, label: 'Team', route: '/team'),
      if (isOwner)
        const _NavItem(
      ...
      if (isOwner)
        const _NavItem(
            icon: Icons.badge_rounded, label: 'HR & Payroll', route: '/hr'),
      if (isOwner)
        const _NavItem(
      ...
      if (isManager)
        const _NavItem(
            ...
            route: '/audit-logs'),
      const _NavItem(
          icon: Icons.replay_rounded, label: 'Returns', route: '/returns'),
      if (isManager)
        const _NavItem(
  ...
  List<_NavItem> _getMoreItems(BuildContext context) {
    ...
    return _getNavItems(context)
        .where((item) => !primary.any((p) => p.route == item.route))
        .toList();
  }
                ...
                children: [
                  ...
                  _buildSheetSection(
                      ...
                      moreItems
                          .where((i) =>
                              i.route == '/reports' ||
                              i.route == '/advanced-analytics' ||
                              i.route == '/ai-assistant' ||
                              i.route == '/team' ||
                              i.route == '/subscription' ||
                              i.route == '/notifications' ||
                              i.route == '/accounting')
                          .toList(),
                      selectedIdx),
                  const SizedBox(height: 8),
  ...
  Widget _buildSheetSection(BuildContext context, String title,
      List<_NavItem> items, int selectedIdx) {
    final theme = Theme.of(context);
        ...
        ...items.map((item) {
          ...
          return ListTile(
            ...
            title: Text(
              ...
              style: TextStyle(
                color:
                    isSelected ? AppColors.accent : theme.colorScheme.onSurface,
                ...
                return Padding(
                  ...
                  child: ListTile(
                    ...
                    trailing: needsUpgrade
                        ? _UpgradeBadge(status: subscriptionStatus)
                        : null,
                    onTap: () {
                      if (isDrawer) Navigator.pop(context);
                      context.go(item.route);
