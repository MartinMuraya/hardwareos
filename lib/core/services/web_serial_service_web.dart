...
class _WebSerialServiceWeb implements WebSerialService {
  ...
  Future<void> requestPort() async {
    ...
    _port = await js_util
        .promiseToFuture(js_util.callMethod(serial, 'requestPort', []));

    final options = js_util.newObject();
    js_util.setProperty(options, 'baudRate', 9600);
    await js_util.promiseToFuture(js_util.callMethod(_port, 'open', [options]));
  }
    ...
    controller = StreamController<String>(onListen: () async {
      ...
      try {
        while (_isReading && js_util.getProperty(_port, 'readable') != null) {
          _reader = js_util.callMethod(
              js_util.getProperty(_port, 'readable'), 'getReader', []);
          try {
            while (_isReading) {
