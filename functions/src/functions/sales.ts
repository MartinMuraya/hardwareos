...
export const createSale = onCall(SECURE_FN_OPTS, async (request) => {
  ...
  // 0. Acquire atomic lock on idempotency key before entering transaction
  if (idempotencyKey) {
    ...
    try {
      ...
      if (err.code === 6 || err.code === "already-exists" || (err.message && err.message.includes("already exists"))) {
        // Key exists: poll briefly for completed transaction result
        for (let attempt = 0; attempt < 6; attempt++) {
          const snap = await idempRef.get();
          if (snap.exists && snap.data()?.status === "COMPLETED" && snap.data()?.result) {
            return { success: true, ...snap.data()!.result };
          }
        ...
        throw new HttpsError("already-exists", "This transaction is currently being processed.");
      }
      throw err;
    }
  }
  ...
  try {
    ...
    result = await db().runTransaction(async (txn) => {
      // Check if idempotency key was already completed in another transaction
      if (idempotencyKey) {
        const idempRef = db().collection("idempotency_keys").doc(idempotencyKey);
        const idempSnap = await txn.get(idempRef);
        if (idempSnap.exists && idempSnap.data()?.status === "COMPLETED" && idempSnap.data()?.result) {
